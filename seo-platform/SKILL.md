...
**Base URL:** `http://openvoiceui:5001/social-api/api/seo`

### Quick Summary
```bash
curl -s "http://openvoiceui:5001/social-api/api/seo/dashboard?tenant=<USER>"
```
Returns: total queries, cost, tools used, top targets, recent queries.

### Keywords
```bash
curl -s "http://openvoiceui:5001/social-api/api/seo/accumulated/keywords?tenant=<USER>&domain=<DOMAIN>&limit=50"
```
Returns: keyword, volume, cpc, difficulty, intent, rank, domain. Sorted by volume.

### Backlinks
```bash
curl -s "http://openvoiceui:5001/social-api/api/seo/accumulated/backlinks?tenant=<USER>&domain=<DOMAIN>"
```
Returns: summary (total, referring_domains, rank, dofollow/nofollow), individual backlinks (url_from, url_to, anchor, type).

### Competitors
```bash
curl -s "http://openvoiceui:5001/social-api/api/seo/accumulated/competitors?tenant=<USER>&domain=<DOMAIN>"
```
Returns: competitor domains with organic keywords, traffic, common keywords, rank.

### Site Audit
```bash
curl -s "http://openvoiceui:5001/social-api/api/seo/accumulated/audits?tenant=<USER>&domain=<DOMAIN>"
```
Returns: onpage_score, pages_count, warnings, errors, notices, checks.

### Local SEO
```bash
curl -s "http://openvoiceui:5001/social-api/api/seo/accumulated/local?tenant=<USER>"
```
Returns: maps rankings, business data, reviews with star distribution.

### Brand Mentions
```bash
curl -s "http://openvoiceui:5001/social-api/api/seo/accumulated/content-monitor?tenant=<USER>&domain=<DOMAIN>"
```
Returns: mentions, news, phrase trends, top sources, sentiment breakdown.

### Alerts
```bash
curl -s "http://openvoiceui:5001/social-api/api/seo/alerts?tenant=<USER>"
```
Returns: active alerts (rank drops, traffic changes, backlink losses).

### Tracked Keywords
```bash
curl -s "http://openvoiceui:5001/social-api/api/seo/tracked-keywords?tenant=<USER>&domain=<DOMAIN>"
```
Returns: keywords being actively monitored.

### Data Pipeline Health
```bash
curl -s "http://openvoiceui:5001/social-api/api/seo/diagnostic?tenant=<USER>&domain=<DOMAIN>"
```
...
curl -s -X POST -H "Content-Type: application/json" \
  -d '{"domain":"<DOMAIN>"}' \
  "http://openvoiceui:5001/social-api/api/seo/report/generate?tenant=<USER>"
```
