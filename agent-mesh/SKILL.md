...
## STALE-CLAIM RE-PROVE (before restating any blocker)

Before you send, restate, or act on a blocker / "X is broken" / "Y is missing" claim that is
**older than 72h**, you MUST:

1. **Re-run the probe against live state.** Not your notes, not the last mesh thread — the
   actual target.
...
If the re-prove shows the blocker is gone, **send a correction naming the original thread
before doing anything else.**

**Why (src-desktop, 2026-W30):** two of three worst time sinks were phantom blockers carried on
unverified probes — a decking "scaffold-only/404s" claim carried for weeks while the site was
live the whole time, and submesh slot-URI layers where each fix looked complete because nothing
proved it wasn't.

**Three more specimens from 2026-07-28, all inside one session, because the failure mode is not
rare:**
- A `netlify.toml [[notifications]]` schema hold froze a 3-site deploy wave for two days. The
  diagnosis behind it had already been tested and RETRACTED — but the hold outlived the
  retraction. Re-prove: 74 sites carry that exact block and 68 are live; the only ones not live
  were the ones held because of it.
- A desk sat frozen for two days awaiting a collision ruling that had been settled the previous
  day. The resolution was never routed back to the frozen party.
- "web_search verified on 26/26 tenants" was a check that the plugin reported `Status: loaded`.
  It never ran a search. Every search had been throwing for weeks.

**Corollary — step 2 is the one people skip.** A probe that CANNOT succeed is worse than no
