...
description: Three.js interaction - raycasting, controls, mouse/touch input, object selection. Use when handling user input, implementing click detection, adding camera controls, or creating interactive 3D experiences. Works in canvas pages via import map.
---

# Three.js Interaction


> **Canvas page setup:** Load `threejs-canvas-integration` skill first for import map template, boilerplate, and asset loading patterns. All `import` statements below resolve via the import map pointing to `cdn.jsdelivr.net`.
...
### PointerLockControls — DOES NOT WORK IN IFRAMES

**WARNING:** `requestPointerLock()` is blocked by browsers inside iframes. Since canvas pages run inside the OpenVoiceUI canvas iframe, PointerLockControls will silently fail — the click handler never fires, no error shown.

**USE OrbitControls instead** for camera control in canvas pages:
```javascript
import { OrbitControls } from "three/addons/controls/OrbitControls.js";
...
**For WASD movement without pointer lock** (keyboard still works in iframes):
```javascript
const keys = {};
...
**Click vs drag detection** (so clicking objects doesn't fight with OrbitControls):
```javascript
...
renderer.domElement.addEventListener("mouseup", e => {
  ...
  if (Math.sqrt(dx*dx + dy*dy) < 5) {
    // Click — handle object interaction via raycasting
  }
});
```
...
- `threejs-canvas-integration` - **START HERE** — Canvas page boilerplate, import maps, asset loading
