...
def render(data: dict, score_result: dict, roadmap: dict, output_path: str, plan: dict = None) -> None:
    ...
    def _plan_section_html(p):
        ...
        # ── REAL coverage of the site's ACTUAL ranked corpus ───────────────────────
        # Mike, 2026-08-12: "i dont get were covered for page one but it says zero?"
        #
        # He was right to distrust it. The panel below this one is scored against
        # `all_keywords`, which is BUILT from keyword suggestions + competitor gaps —
        ...
        _rk_total = int(data.get("kw_total", 0) or 0)
        _rk_top3  = int(data.get("kw_top3", 0) or 0)
        _rk_4_10  = int(data.get("kw_4_10", 0) or 0)
        _rk_11_20 = int(data.get("kw_11_20", 0) or 0)
        _rk_21_50 = int(data.get("kw_21_50", 0) or 0)
        _rk_51_100 = int(data.get("kw_51_100", 0) or 0)
        _rk_p1    = _rk_top3 + _rk_4_10
        _avg_pos  = data.get("avg_position", 0) or 0
        real_cov_html = ""
        if _rk_total > 0:
            real_cov_html = (
                f'<h3 class="text-base font-bold uppercase tracking-wider mb-3 mt-2" '
                f'style="color:var(--brand-muted);font-family:var(--font-ui)">'
                f'Where You Rank Today — Your {_rk_total:,} Ranked Keywords</h3>'
                f'<p style="font-size:0.85rem;color:var(--brand-muted);margin:0 0 12px">'
                f'Every keyword your site already ranks for in Google\'s organic index, by position. '
                f'This is what you have; the opportunity set below is what you don\'t.</p>'
                f'<div style="display:flex;gap:12px;flex-wrap:wrap;margin-bottom:8px">'
                f'<div class="bl-kpi ok" style="flex:1;min-width:110px"><div class="bl-kpi-label">Page 1 (1–10)</div><div class="bl-kpi-value">{_rk_p1}</div></div>'
                f'<div class="bl-kpi warn" style="flex:1;min-width:110px"><div class="bl-kpi-label">Page 2 (11–20)</div><div class="bl-kpi-value">{_rk_11_20}</div></div>'
                f'<div class="bl-kpi" style="flex:1;min-width:110px"><div class="bl-kpi-label">21–50</div><div class="bl-kpi-value">{_rk_21_50}</div></div>'
                f'<div class="bl-kpi bad" style="flex:1;min-width:110px"><div class="bl-kpi-label">51–100</div><div class="bl-kpi-value">{_rk_51_100}</div></div>'
                f'<div class="bl-kpi" style="flex:1;min-width:110px"><div class="bl-kpi-label">Avg position</div><div class="bl-kpi-value">{_avg_pos}</div></div></div>'
                f'<p style="font-size:0.85rem;color:var(--brand-muted);margin:0 0 22px">'
                f'{"<strong>" + str(_rk_p1) + " on page one.</strong> " if _rk_p1 else "<strong>Nothing on page one yet.</strong> "}'
                f'{str(_rk_51_100) + " sit at position 51–100 — ranked, but effectively unseen. Those are the cheapest wins: pages that already exist and already rank, needing depth rather than creation." if _rk_51_100 else ""}'
                f'</p>'
            )
        ...
        cov_html = (
            ...
            f'<h3 class="text-base font-bold uppercase tracking-wider mb-3 mt-2" '
            f'style="color:var(--brand-muted);font-family:var(--font-ui)">'
            f'Opportunity Set — {cu} High-Value Keywords You Don\'t Yet Rank For</h3>'
            f'<p style="font-size:0.85rem;color:var(--brand-muted);margin:0 0 12px">'
            f'Built from keyword research plus keywords your competitors rank for and you don\'t. '
            f'By design these are keywords you are <em>not</em> currently covering — they are the '
            f'build target, not a scorecard of your current performance.</p>'
            f'<div style="display:flex;gap:12px;flex-wrap:wrap;margin-bottom:14px">'
            f'<div class="bl-kpi ok" style="flex:1;min-width:120px"><div class="bl-kpi-label">✓ Already top 10</div><div class="bl-kpi-value">{cc}</div></div>'
            f'<div class="bl-kpi warn" style="flex:1;min-width:120px"><div class="bl-kpi-label">⚠ Within reach (11–30)</div><div class="bl-kpi-value">{cp}</div></div>'
            f'<div class="bl-kpi bad" style="flex:1;min-width:120px"><div class="bl-kpi-label">✗ To capture</div><div class="bl-kpi-value">{cm}</div></div>'
            f'<div class="bl-kpi" style="flex:1;min-width:120px"><div class="bl-kpi-label">Opportunity set</div><div class="bl-kpi-value">{cu}</div></div></div>'
  ...
  <section id="build-plan">
    ...
    # ── Map pack table ────────────────────────────────────────────────────────
    # Prefer the richer per-(city,keyword) rows when fetch_local provides them — they
    # show the SPECIFIC vertical term the business actually ranks for (e.g. "spray foam
    # equipment #1"), not just the single generic detected service. Falls back to the
    # legacy city→position dict. (2026-06-29.)
    map_rows = ""
    map_pack_rows = data.get("map_pack_rows") or []
    if map_pack_rows:
        for r in map_pack_rows:
            pos = r.get("position")
            pos_str = f"#{pos}" if pos else "Not ranked"
            row_cls = ' class="us"' if pos and pos <= 10 else ""
            map_rows += (f"<tr{row_cls}><td>{_escape(r.get('city',''))}</td>"
                         f"<td>{_escape(r.get('keyword', service))}</td>"
  ...
  <section id="local-seo">
    ...
    {('<h3 class="text-base font-bold uppercase tracking-wider mb-3" style="color:var(--brand-muted); font-family: var(--font-ui);">Recent Reviews</h3>'
      ...
      + "".join(
          ...
          f'<span style="color:var(--brand-muted); margin-left:8px;">{_escape(str(rv.get("author") or "Customer"))}</span></div>'
          f'<div style="color:var(--brand-text-strong); font-size:14px; margin-top:4px;">{_escape(str(rv.get("text") or ""))}</div>'
          f'</div>'
          for rv in (data.get("recent_reviews") or [])[:3]
        )
      + "</div>") if (data.get("recent_reviews")) else ""}
