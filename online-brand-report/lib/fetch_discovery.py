...
def _geo_conflict(text: str, city: str, state: str) -> str:
    ...
    DELIBERATELY CONSERVATIVE — flags only on an EXPLICIT conflicting state, never on the
    absence of a location. A press mention that never names a city must still count; the
    failure mode we refuse is dropping real mentions to look tidy. Absence of evidence is
    ...
    raw = text or ""
    t = " " + raw.lower() + " "
    mine = state.strip().upper()
    city_l = (city or "").strip().lower()
    if city_l and city_l in t:
        return ""          # names the client's own city → ours, regardless of anything else

    # Matching is deliberately narrow, because a BARE two-letter token is not a state:
    # "IN", "OR", "ME", "OK", "HI", "LA", "PA", "DE", "MA", "ID", "CO", "AL", "AR" are all
    # ordinary English words. The first cut of this function matched a bare lowercase code
    # and reported "Auto Glass IN Rochester, NY" as a conflict with **Indiana** — it dropped
    # the right row for the wrong reason, which is the same confident-wrong-word defect this
    # guard exists to prevent. Caught only because the unit test asserted WHICH state came
    # back, not merely that something was dropped.
    #   - full state NAMES match case-insensitively (unambiguous)
    #   - two-letter CODES match only in the canonical "City, ST" shape: a comma, optional
    #     space, the code in UPPERCASE, then a non-letter. Uses the original-case string.
    import re as _re
    for name, code in _US_STATES.items():
        ...
        if f" {name} " in t:
            return code
        if _re.search(rf",\s*{code}(?![A-Za-z])", raw):
            return code
    return ""


def _other_mentions(items: list, client_domain: str, slugs: list[str],
    ...
    for it in items:
        ...
        if host in seen:
            continue
        # Same-NAME-different-BUSINESS guard (see _geo_conflict). Only fires on an
        # explicitly conflicting state, so mentions with no location survive.
        _conflict = _geo_conflict(f"{title} {url}", city, state)
        if _conflict:
            print(f"[INFO] mention '{title[:50]}' @ {host} names {_conflict}, client is "
                  f"{state} — dropping same-name-different-business", file=sys.stderr)
            continue
        seen.add(host)
...
def fetch_discovery(domain: str, brand_name: str, city: str = "", state: str = "",
    ...
    out["other_mentions"]  = _other_mentions(all_items, domain, slugs, city, state)
