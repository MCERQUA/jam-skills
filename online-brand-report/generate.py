...
def main():
    ...
    try:
        ...
        _sigs = {
            ...
            # GBP / GMB — gbp_found requires a GBP lookup to have actually SEEN a listing.
            # It used to also accept `gmb_address or gmb_phone` on the reasoning that either
            # "proves existence". It does not: those two fields are ALSO populated from the
            ...
            # Multi-location: every listing the brand owns + the audit flags that only a
            # full enumeration can produce (see lib/fetch_local._fetch_gbp_locations).
            "gbp_locations":            data.get("gbp_locations", []),
            "gbp_location_count":       int(data.get("gbp_location_count", 0) or 0),
            "gbp_is_multi_location":    int(data.get("gbp_location_count", 0) or 0) > 1,
            "gbp_zero_review_locations": data.get("gbp_zero_review_locations", []),
            "gbp_name_variants":        data.get("gbp_name_variants", []),
            "gbp_nap_name_consistent":  bool(data.get("gbp_nap_name_consistent", True)),
            "gbp_listing_urls":         data.get("gbp_listing_urls", []),
            "gbp_all_urls_canonical":   bool(data.get("gbp_all_urls_canonical", False)),
