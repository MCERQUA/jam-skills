...
**RULE 0 — Determine the design source FIRST.** Read `intake.creatorStyle`, `intake.stitchScreens` and `intake.designTemplate` BEFORE any other phase decision. The result picks one of four paths:

| TIER 1 winner | Trigger | What it means |
|---|---|---|
| **1.A-CREATOR — Chosen Creator style** | `intake.creatorStyle` non-empty (the Website Setup 4-style chooser; `stitchScreens` will contain one `partial:true` home screen in `intake.stitchProjectId`) | The client CHOSE this homepage design from the 4-style drawer — it is the LITERAL homepage blueprint AND the style contract for the whole site. (1) FETCH the chosen screen via `get_screen` (`intake.creatorStyle.projectId` + `.screenId`) and convert it as the home page — do NOT redesign, reorder, or inject sections into it. (2) Every ADDITIONAL page from the plan (services, subservices, areas, about, contact, FAQ, blog…) MUST be generated with `generate_screen_from_text` **inside the SAME Stitch project** (`intake.stitchProjectId`) so Stitch keeps the style consistent — build each page's prompt from the Phase 1/2 research + plan (its brief: H1, keywords, sections) PLUS an explicit style-continuation preamble naming the home screen's palette, type, and band rhythm (parse via `tools/parse-stitch-html.py` on the fetched home HTML). (3) Palette/tokens come from the chosen home screen (RULE 1 1.A rules apply). (4) The Stitch fidelity gate (Phase 7) applies to the home page vs the chosen screen. The 1.A prohibition on adding pages does NOT apply — this tier is "partial screens supplied, complete the set in-style". |
| **1.A — Stitch supplied** | `intake.stitchScreens` non-empty AND no `creatorStyle` | The user gave you existing Stitch screens by ID. FETCH them via `get_screen` and use them as the LITERAL blueprint. DO NOT call `create_project` or `generate_screen_from_text`. DO NOT add pages that aren't in the supplied screens. DO NOT mix in any "PAGE COMPOSITION BY BUSINESS TYPE" boilerplate from this doc. |
